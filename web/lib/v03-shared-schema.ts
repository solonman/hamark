...
export async function isV03SharedSchemaReady(db: DbClient = getDbClient()) {
  const row = await db.prepare(
    `SELECT
      to_regclass('public.v03_collaboration_streams') IS NOT NULL AS streams,
      to_regclass('public.v03_collaboration_baselines') IS NOT NULL AS baselines,
      to_regclass('public.v03_collaboration_rounds') IS NOT NULL AS rounds,
      to_regclass('public.v03_collaboration_revision_events') IS NOT NULL AS revisions,
      EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'analysis_comments'
          AND column_name = 'collaboration_round_id'
      ) AS comments_linked`,
  ).first<Record<string, boolean>>();
  return Boolean(
    row?.streams && row.baselines && row.rounds && row.revisions && row.comments_linked,
  );
}

export async function installV03SharedSchema(db: DbClient = getDbClient()) {
  await db.batch(statements.map((statement) => db.prepare(statement)));
  if (!(await isV03SharedSchemaReady(db))) {
    throw new Error("V0.3 shared collaboration schema verification failed");
  }
}
