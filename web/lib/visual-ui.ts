...
//
// 案例简介（textBody）标为选填：GET /api/visual-cases 的列表项（VisualCaseListItem）
// 里没有这个字段——约定文件里没有它，列表接口按规格五也只承诺「载体计数、封面…」，
// 不含可能长达 2 万字的正文，这是合理的：列表一次读全量，把每条案例的正文都塞进去
// 会让列表接口的响应体膨胀。库首页调用时这一项就是空的，只在（将来）传了 textBody
// 的场景下才会被搜到。这算是缺口 3：约定文件没给列表项一个「案例简介」字段，
// demo 的搜索口径却包含它——已在最终报告里向负责人说明。
// ---------------------------------------------------------------------------
