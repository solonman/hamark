...
import { isVisualFeatureEnabled, type VisualAddAssetsRequest } from "@/lib/visual-contract";
import { visualFeatureDisabledResponse, VisualServiceError } from "@/lib/visual-model";
import { addVisualAssets } from "@/lib/visual-server";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const body = (await request.json()) as Partial<VisualAddAssetsRequest>;
  try {
    const result = await addVisualAssets(getDbClient(), {
      caseId: id,
      assets: body.assets ?? [],
    ...
    return Response.json(
      { error: error instanceof Error ? error.message : "追加素材失败，请稍后重试。" },
      { status },
    );
  }
}
