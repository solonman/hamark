...
function TextAreaField({
  ...
  return (
    <label className={wide ? "editor-wide" : ""}>
      <span>{label}</span>
      <small>{hint}</small>
    ...
    <>
      <section className="worksheet-section" id="core">
        ...
        <div className="core-editor-grid v03-core-grid">
          <TextAreaField label="商业意图" hint="这支片最终要改变谁的什么认知或行为？" value={draft.commercialIntent} onChange={(value) => onChange({ ...draft, commercialIntent: value })} />
          <TextAreaField label="故事梗概" hint="不评价，把起点、进展和结果准确说清。" value={draft.synopsis} onChange={(value) => onChange({ ...draft, synopsis: value })} rows={5} />
          <TextAreaField label="创意母题" hint="用一句话写出最核心的创意命题。" value={draft.creativeTheme} onChange={(value) => onChange({ ...draft, creativeTheme: value })} />
          <TextAreaField label="创意按钮" hint="本片特有的设定、关系变化、意义判断、语言操作或视听规则是什么？" value={structure.creativeButton} onChange={(value) => updateStructure({ creativeButton: value })} rows={5} />
          <TextAreaField label="创意机制具体句" hint="先用这个案例自己的语言，写清按钮怎样产生作用。" value={structure.mechanismStatement} onChange={(value) => updateStructure({ mechanismStatement: value })} rows={5} />
          <label>
            <span>机制二级归类</span>
            ...
            <select value={structure.mechanismPrimary} onChange={(event) => updateStructure({ mechanismPrimary: event.target.value, mechanismAuxiliary: structure.mechanismAuxiliary.filter((value) => value !== event.target.value) })}>
              ...
              {mechanismOptions.map((option) => <option value={option} key={option}>{option}</option>)}
            </select>
          </label>
          <div className="editor-wide v03-inline-field">
            <span>辅助机制（可选 0—2 项）</span>
            <ToggleList options={mechanismOptions.map((value) => ({ value, label: value }))} values={structure.mechanismAuxiliary} disabledValue={structure.mechanismPrimary} limit={2} onChange={(values) => updateStructure({ mechanismAuxiliary: values })} />
          </div>
          ...
          <label><span>故事原型</span><small>允许复合、待定和开放说明。</small><input list="v03-story-archetype" value={structure.storyArchetype} onChange={(event) => updateStructure({ storyArchetype: event.target.value })} /><datalist id="v03-story-archetype">{storyArchetypeOptions.map((value) => <option value={value} key={value} />)}</datalist></label>
          <div className="editor-wide v03-formation-card">
            <span>创意如何形成</span>
            <small>人工选择全片主形成方式；不要输入镜头编号。</small>
            ...
            <ToggleList options={formationOptions.map((option) => ({ value: option.value, label: option.label }))} values={structure.formationAuxiliary} disabledValue={structure.formationPrimary} limit={2} onChange={(values) => updateStructure({ formationAuxiliary: values as CreativeStructureDraft["formationAuxiliary"] })} />
            {structure.formationPrimary === "LOCAL_TRIGGER" ? <div className="v03-group-picker"><span>可选关键桥段（按标题选择）</span>{(draft.shotGroups ?? []).map((group) => <label key={group.id}><input type="checkbox" checked={structure.formationRelatedGroupIds.includes(group.id)} onChange={() => updateStructure({ formationRelatedGroupIds: structure.formationRelatedGroupIds.includes(group.id) ? structure.formationRelatedGroupIds.filter((id) => id !== group.id) : [...structure.formationRelatedGroupIds, group.id] })} />{group.title || "未命名桥段"}</label>)}</div> : null}
          </div>
          <TextAreaField label="形成说明" hint={structure.formationPrimary === "COMPOSITE" ? "用 1—3 句话说明各形成方式的分工。" : "用 1—3 句话说明创意在全片中怎样形成。"} value={structure.formationStatement} onChange={(value) => updateStructure({ formationStatement: value })} rows={5} wide />
          <TextAreaField label="创意承重载体" hint="哪些故事、文案、动作、视听规则或品牌要素真正在承重？" value={structure.creativeCarriers} onChange={(value) => updateStructure({ creativeCarriers: value })} />
          <TextAreaField label="创意成立条件" hint="观众需要接受什么关系、规则或前提？" value={structure.establishmentConditions} onChange={(value) => updateStructure({ establishmentConditions: value })} />
          <TextAreaField label="成片强度来源" hint="这支成片的力量主要来自哪些表达层？" value={structure.strengthSources} onChange={(value) => updateStructure({ strengthSources: value })} />
          ...
          {structure.conditionFlags.unconventionalWorld ? <TextAreaField label="成立契约" hint="这个非常规世界如何让观众愿意接受？" value={structure.acceptanceContract} onChange={(value) => updateStructure({ acceptanceContract: value })} wide /> : null}
          {structure.conditionFlags.audiovisualCarriesIdea ? <TextAreaField label="视听机制具体操作" hint="不写抽象风格，写清视听规则具体怎样使创意发生。" value={structure.audiovisualMechanism} onChange={(value) => updateStructure({ audiovisualMechanism: value })} wide /> : null}
          {(structure.primaryCreativePath === "INTERESTING" || structure.conditionFlags.interestingLoadBearing) ? <TextAreaField label="信息释放／转折结构" hint="写清原始预期、偏离、揭示与重释怎样被组织。" value={structure.informationReleaseTurning} onChange={(value) => updateStructure({ informationReleaseTurning: value })} wide /> : null}
          <TextAreaField label="全篇创意总结" hint="综合说明商业转译、母题、按钮、感知实现与品牌落点如何共同成立。" value={draft.summary} onChange={(value) => onChange({ ...draft, summary: value, shotCommentary: value })} rows={7} wide />
        </div>
        <details className="v03-optional-metadata">
          <summary>非阻断结构化元数据 <span>可选·不计入发布完整度</span></summary>
          <p>这些项仅用于检索与后续 AI 校验，不恢复成 19 项必填表。</p>
          <div className="v03-metadata-grid">{optionalFields.map((field) => { const answer = draft.fields.find((item) => item.code === field.code)?.answer ?? ""; return <label key={field.code}><span>{field.code} {field.name}</span><small>{field.question}</small><input list={`metadata-${field.code}`} value={answer} onChange={(event) => updateOptionalField(field.code, event.target.value)} /><datalist id={`metadata-${field.code}`}>{field.options.map((option) => <option value={option.value} key={option.value} />)}</datalist></label>; })}</div>
        </details>
      </section>
      ...
      <section className="worksheet-section" id="path">
        ...
        <div className="v03-path-editor">
          <div><span>主导路径（必选 1 项）</span><div className="v03-radio-grid path">{creativePathOptions.map((option) => <label key={option.value} className={structure.primaryCreativePath === option.value ? "is-selected" : ""}><input type="radio" name="creative-path" checked={structure.primaryCreativePath === option.value} onChange={() => updateStructure({ primaryCreativePath: option.value, auxiliaryCreativePaths: structure.auxiliaryCreativePaths.filter((path) => path !== option.value) })} /><strong>{option.label}</strong></label>)}</div></div>
          <div><span>辅助路径（可选 0—2 项）</span><ToggleList options={creativePathOptions} values={structure.auxiliaryCreativePaths} disabledValue={structure.primaryCreativePath} limit={2} onChange={(values) => updateStructure({ auxiliaryCreativePaths: values as CreativePath[] })} /></div>
          <TextAreaField label="复合态判断" hint="说明为什么有一条主导路径，而不是三类并列。" value={structure.compositeStateReason} onChange={(value) => updateStructure({ compositeStateReason: value })} rows={5} wide />
      ...
      <section className="worksheet-section" id="grade">
        ...
        <div className="core-editor-grid"><TextAreaField label="自评理由" hint="写清你为什么给出这个等级。" value={structure.creativeGradeReason} onChange={(value) => updateStructure({ creativeGradeReason: value })} rows={5} wide /></div>
