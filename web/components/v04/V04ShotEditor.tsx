...
export default function V04ShotEditor({ shot, number, groupId, groupTargets, previousShot, readOnly, onChange, onMoveUp, onMoveDown, onMoveTo, onDragStart, onDragEnd }: {
  shot: V04UiShot;
  number: number;
  groupId: string;
        ...
        {V04_UI_SHOT_FIELDS.map(({ key, label }) => {
          ...
          return (
            <label key={key} id={v04ShotFieldTargetId(shot.id, key)} className={`${styles.shotField} ${key === "visualContent" ? styles.shotFieldWide : ""}`}>
              <span>{label}{repeatable && number > 1 && <button type="button" disabled={readOnly || !canRepeat} onClick={() => previousShot && onChange(key, previousShot[key])}>同上</button>}</span>
