...
export default function V04VideoPlayer({ caseId, title, surface }: { caseId: string; title: string; surface: "detail" | "workspace" }) {
      ...
      {video.minimized ? (
        ...
        <>
          <div className={styles.videoStage}>
            <video ref={ref} controls preload="metadata" aria-label={`${title} 视频播放器`} onTimeUpdate={(event) => updateVideo({ currentTime: event.currentTarget.currentTime })} />
            <div className={styles.videoPlaceholder}><span>V1.9 FIXTURE VIDEO</span><strong>{title}</strong><small>Mock 媒体壳 · 不连接生产文件</small></div>
          </div>
