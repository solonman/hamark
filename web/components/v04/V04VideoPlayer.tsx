...
export default function V04VideoPlayer({ caseId, title, surface, media }: { caseId: string; title: string; surface: "detail" | "workspace"; media?: V04UiMediaReference | null }) {
      ...
      {video.minimized ? (
        ...
        <>
          <div className={styles.videoStage}>
            <video ref={ref} controls preload="metadata" src={media?.streamPath} aria-label={`${title} 视频播放器`} onTimeUpdate={(event) => updateVideo({ currentTime: event.currentTarget.currentTime })} />
