...
export default function V04DetailClient({ videoId, viewerName, embedded = false, showVideo = true, navigation = shadowNavigation(videoId) }: { videoId: string; viewerName: string; embedded?: boolean; showVideo?: boolean; navigation?: V04DetailNavigation }) {
  ...
  if (!item || !model) return <section className={shell} data-v04-page="detail"><section className={styles.emptyState}><h2>正在读取案例成果…</h2></section></section>;
  const content = <>
    {!embedded && <header className={styles.siteHeader} data-v04-fixed-header><Link href={navigation.libraryHref} className={styles.brandWordmark}><b>R:</b><span>RE:VERSE</span><small>反写</small></Link><nav className={styles.siteNav}><Link href={navigation.libraryHref}>案例库</Link><Link href={navigation.detailHref} className={styles.activeNav}>{navigation.detailLabel ?? "案例分析"}</Link></nav><div className={styles.siteUtilities}><Link href={navigation.workspaceHref}>{draft ? "编辑工作稿" : "开始公共工作稿"}</Link><button type="button" onClick={() => setHistory(true)}>历史版本</button>{navigation.managementHref ? <Link href={navigation.managementHref}>视频管理</Link> : null}<span>{viewerName}</span></div></header>}
