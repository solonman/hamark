...
import { blankV04Shot, evaluateV04FixturePublication, moveV04Shot, nextV04Timecode, numberedV04Shots } from "@/lib/v04-ui-client-state";
...
function Field({ id, label, value, disabled, tall = false, onChange }: { id: string; label: string; value: string; disabled: boolean; tall?: boolean; onChange: (value: string) => void }) {
  return <label className={styles.formField} id={id}><span>{label}<em>发布必填</em></span>{tall ? <textarea value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} /> : <input value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} />}</label>;
}
...
export default function V04WorkspaceClient({ item, viewerName }: { item: V04UiCase; viewerName: string }) {
  ...
  const locateMissing = (id: string) => { const target = document.getElementById(id); target?.scrollIntoView({ behavior: "smooth", block: "center" }); target?.querySelector<HTMLElement>("input,textarea,button")?.focus({ preventScroll: true }); target?.classList.add(styles.located); window.setTimeout(() => target?.classList.remove(styles.located), 1500); };
  const addShot = (groupId: string) => updateGroup(groupId, (group) => { const previous = draft.shotGroups.flatMap((entry) => entry.shots).at(-1); const next = blankV04Shot(`shot-fixture-${Date.now()}`); next.startTime = nextV04Timecode(previous?.endTime ?? ""); group.shots.push(next); window.setTimeout(() => locateMissing(`shot-${next.id}`), 0); });
  const addGroupAfter = (groupId: string) => updateDraft((next) => { const index = next.shotGroups.findIndex((group) => group.id === groupId); const id = `bridge-fixture-${Date.now()}`; next.shotGroups.splice(index + 1, 0, { id, title: "", primaryRole: { selectedOptionIds: [], customText: "", vocabularyVersion: V04_VOCABULARY_VERSION }, auxiliaryRole: { selectedOptionIds: [], customText: "", vocabularyVersion: V04_VOCABULARY_VERSION }, creativeDescription: "", shots: [blankV04Shot(`shot-${id}-01`)] }); window.setTimeout(() => locateMissing(`group-${id}`), 0); });
  const moveShotBy = (shotId: string, delta: number) => updateDraft((next) => { const group = next.shotGroups.find((entry) => entry.shots.some((shot) => shot.id === shotId)); if (!group) return; const index = group.shots.findIndex((shot) => shot.id === shotId); next.shotGroups = moveV04Shot(next.shotGroups, shotId, group.id, index + delta); window.setTimeout(() => locateMissing(`shot-${shotId}`), 0); });
  const moveShotTo = (shotId: string, groupId: string) => updateDraft((next) => { const target = next.shotGroups.find((group) => group.id === groupId); if (!target) return; next.shotGroups = moveV04Shot(next.shotGroups, shotId, groupId, target.shots.length); window.setTimeout(() => locateMissing(`shot-${shotId}`), 0); });
  ...
  return <main className={styles.surface} data-v04-page="workspace">
    <header className={styles.productHeader}><Link href="/v04-shadow" className={styles.wordmark}>← 案例库</Link><nav><Link href={`/v04-shadow/videos/${item.id}`}>只读成果</Link><Link href={`/v04-shadow/videos/${item.id}/workspace`}>编辑工作稿</Link><button onClick={() => setHistory(true)}>历史</button></nav><div className={styles.saveCluster}><span>{saveLabel}</span><button onPointerDown={(event) => event.preventDefault()} onClick={manualSave} disabled={!canEdit || saveState === "saving"}>保存</button></div></header>
        ...
        <fieldset disabled={!canEdit} className={styles.editorFieldset} onFocusCapture={(event) => {
          ...
          <section className={styles.editorModule} id="module-1"><header><small>第一模块</small><h2>第一模块｜脚本反写</h2><p>先按桥段组织，再逐镜还原；每个镜头保持 12 项独立科目。</p></header>
            {draft.shotGroups.map((group, groupIndex) => <article className={`${styles.bridgeCard} ${draggedShotId ? styles.isDropTarget : ""}`} key={group.id} id={`group-${group.id}`} onDragOver={(event) => { if (!draggedShotId) return; event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={(event) => { event.preventDefault(); if (!draggedShotId) return; moveShotTo(draggedShotId, group.id); setDraggedShotId(null); }}>
              <header className={styles.bridgeHeader}><b>桥段 {String(groupIndex + 1).padStart(2, "0")}</b><input aria-label="桥段名称" value={group.title} onChange={(event) => updateGroup(group.id, (next) => { next.title = event.target.value; })} placeholder="桥段名称" /></header>
              <div className={styles.bridgeChoices}><V04ChoiceField label="桥段主创意作用" value={group.primaryRole} options={V04_UI_BRIDGE_OPTIONS} customLabel="自定义主创意作用" disabled={!canEdit} onChange={(value) => updateGroup(group.id, (next) => { next.primaryRole = value; })} /><V04ChoiceField label="桥段辅助创意作用" value={group.auxiliaryRole} options={V04_UI_BRIDGE_OPTIONS} customLabel="自定义辅助创意作用" multiple disabled={!canEdit} onChange={(value) => updateGroup(group.id, (next) => { next.auxiliaryRole = value; })} /></div>
              <Field id={`field-${group.id}-description`} label="本桥段关键创意描述" value={group.creativeDescription} disabled={!canEdit} tall onChange={(value) => updateGroup(group.id, (next) => { next.creativeDescription = value; })} />
          ...
          <section className={`${styles.editorModule} ${collapsed.has(2) ? styles.collapsed : ""}`} id="module-2"><header><small>第二模块</small><h2>第二模块｜全片事实与核心判断</h2><button type="button" onClick={() => toggleModule(2)}>{collapsed.has(2) ? "展开" : "收起"}</button></header>{!collapsed.has(2) && <>
            ...
            <div id="field-primaryMechanism"><V04ChoiceField label="创意主导手法及机制" value={draft.primaryMechanism} options={V04_UI_MECHANISM_OPTIONS} customLabel="自定义通用机制" showAdvanced={draft.primaryMechanism.selectedOptionIds.includes("PENDING_NEW_MECHANISM")} disabled={!canEdit} onChange={(value) => updateChoice("primaryMechanism", value)} /></div>
