...
function Field({ id, label, value, readOnly, tall = false, required = true, onChange }: { id: string; label: string; value: string; readOnly: boolean; tall?: boolean; required?: boolean; onChange: (value: string) => void }) {
  const controlId = `${id}-control`;
  return <label className={styles.formField} id={id} htmlFor={controlId}><span>{label}{required && <em>发布必填</em>}</span>{tall ? <textarea id={controlId} data-v04-primary-focus value={value} readOnly={readOnly} onChange={(event) => onChange(event.target.value)} /> : <input id={controlId} data-v04-primary-focus value={value} readOnly={readOnly} onChange={(event) => onChange(event.target.value)} />}</label>;
}
...
export default function V04WorkspaceClient({
  ...
  return <main className={styles.surface} data-v04-page="workspace">
      ...
      <div className={styles.editorColumn}>
        <div aria-readonly={!canEdit} className={`${styles.editorFieldset} ${!canEdit ? styles.readOnlyEditor : ""}`} onFocusCapture={(event) => {
          ...
          <section className={styles.editorModule} id="module-1"><header><small>第一模块</small><h2>第一模块｜脚本反写</h2><p>先按桥段组织，再逐镜还原；每个镜头保持 12 项独立科目。</p></header>
            {!draft.shotGroups.length && <div className={styles.bridgeActions}><button type="button" disabled={!canEdit} onClick={() => updateDraft((next) => { const id = `bridge-${crypto.randomUUID()}`; next.shotGroups.push({ id, title: "", primaryRole: { selectedOptionIds: [], customText: "", vocabularyVersion: V04_VOCABULARY_VERSION }, auxiliaryRole: { selectedOptionIds: [], customText: "", vocabularyVersion: V04_VOCABULARY_VERSION }, creativeDescription: "", shots: [blankV04Shot(`shot-${crypto.randomUUID()}`)] }); window.setTimeout(() => locate(v04GroupTitleTargetId(id)), 0); })}>＋ 新增第一个桥段</button></div>}
    ...
    <V04CommentDrawer videoId={item.id} open={comments} onClose={() => setComments(false)} onLocate={locate} draft={draft} readOnly={!canEdit} />
