...
test("contract lifecycle route is POST-only, same-origin protected and the short production gate is isolated", () => {
  ...
  assert.deepEqual(deployment.env, { V04_CONTRACT_ACTIVATE_ENABLED: "true" });
});
