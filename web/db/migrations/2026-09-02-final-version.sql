...
-- 与 db/final-version-schema.ts 中的 FINAL_VERSION_SCHEMA_STATEMENTS 一一对应。
--
...
--
-- 本迁移是附加式的：新增两张表、两个索引、两张表的 RLS 收口；
-- 另外给 analysis_versions 加一个可空默认列（base_is_final），
-- 给 analysis_version_comments 去掉 version_id 上的外键（因为最终版的 id
-- 不在 analysis_versions 里，写在最终版上的评论会被这条旧约束挡住）。
-- 不删除、不改写任何既有数据。可重复执行。
...
CREATE INDEX IF NOT EXISTS analysis_final_intakes_final_seq_idx
  ON analysis_final_intakes (final_id, seq);

CREATE INDEX IF NOT EXISTS analysis_final_intakes_final_target_idx
  ON analysis_final_intakes (final_id, target_key);

-- spec 五、13:「基于最终版」手动创建的版本标记。
ALTER TABLE analysis_versions
  ADD COLUMN IF NOT EXISTS base_is_final BOOLEAN NOT NULL DEFAULT false;

-- 最终版的 id 不在 analysis_versions 里，这条旧外键会挡住写在最终版上的评论。
-- 服务端改为自行校验 version_id 属于该案例（普通版本或最终版）。
ALTER TABLE analysis_version_comments
  DROP CONSTRAINT IF EXISTS analysis_version_comments_version_id_fkey;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    EXECUTE 'REVOKE ALL ON TABLE analysis_final_versions, analysis_final_intakes FROM anon';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated') THEN
    EXECUTE 'REVOKE ALL ON TABLE analysis_final_versions, analysis_final_intakes FROM authenticated';
  END IF;
END
